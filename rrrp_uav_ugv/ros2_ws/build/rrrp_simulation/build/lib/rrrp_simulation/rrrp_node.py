        ...
        if self.ugv_pos is not None and getattr(self, 'ugv_path', None):
            if self.ugv_waypoint_index >= 0 and self.ugv_waypoint_index < len(self.ugv_path):
                ...
                if dist < 0.5: # Reduced to 0.5 so it drives right up to the tree!
                    ...
                    # --- THE FIX: High-Speed Continuous Pure Pursuit ---
                    # To follow the path smoothly and move like a fast vehicle:
                    
                    # 1. Aggressive, smooth proportional steering
                    ugv_cmd.angular.z = max(-3.0, min(3.0, err_yaw * 3.5))
                    
                    # 2. Continuous speed scaling:
                    # - If off-target by > 11 degrees, creep forward at 0.4 m/s to allow tight turns without friction lock
                    # - If aligned, blast forward at up to 5.0 m/s!
                    alignment = max(0.0, math.cos(err_yaw))
                    
                    if abs(err_yaw) > 0.2:
                        ugv_cmd.linear.x = 0.4 
                    else:
                        # Ramps up exponentially as it aligns perfectly
                        ugv_cmd.linear.x = 0.4 + (4.6 * (alignment ** 10))
            else:
                # Finished path
